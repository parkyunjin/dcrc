...
# Function to generate a DCSBM graph with gamma, alpha, and B as inputs, and an optional seed
generate_dcsbm_pareto <- function(n, gamma_vector, x_min, beta, B, seed = NULL) {
  # Set the random seed if provided
  if (!is.null(seed)) {
    set.seed(seed)
  }
  ...
  for (i in 1:n) {
    for (j in i:n) {  # Only upper triangle to avoid duplicates
      ...
      # Calculate the probability of an edge
      p_ij <- min(1, B[block_i, block_j] * theta[i] * theta[j]) # Just in case where probability is larger than 1
...
# Function to generate a DCSBM graph with gamma, alpha, and B as inputs, and an optional seed
generate_dcsbm_normal <- function(n, gamma_vector, mu, sigma, B, seed = NULL) {
  # Set the random seed if provided
  if (!is.null(seed)) {
    set.seed(seed)
  }
